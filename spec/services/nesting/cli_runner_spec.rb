...
RSpec.describe Nesting::CliRunner do
  ...
  describe ".call [REQ-FIT-CLI-001]" do
    ...
    it "kills the Open3 child process when cancellation is requested during CLI execution" do
    ...
    it "runs invoke without a cancel_check callback" do
    ...
    it "skips nested output attachment when nested.dxf is absent" do
      described_class.call(nesting_run: nesting_run, invoke: ->(_work_dir, _config_path) { 0 })

      expect(project.reload.nested_dxf).not_to be_attached
    end

    it "does not finalize runs that already left processing" do
  ...
  describe ".finalize_from_work_dir! [REQ-FIT-CLI-001]" do
    it "returns false when the work directory is missing" do
      expect(described_class.finalize_from_work_dir!(nesting_run: nesting_run)).to be(false)
    end

    it "returns false when report.json is empty" do
