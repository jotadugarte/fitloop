<task_session>
  ...
  <implementation_plan>
    ...
    <step id="6" status="complete">Ejecutar la suite de pruebas completa nuevamente para asegurar que el cambio no haya introducido regresiones [REQ-FIT-QA-001]</step>
  </implementation_plan>
  ...
  <working_notes>
    ...
    - Actualmente, `bin/dev` ejecuta Puma usando Active Job en modo `:async` (en proceso). Esto enmascara problemas de concurrencia, carga de esquemas de colas y fallos de trabajadores distribuidos que sí ocurren en el coolify (donde se usa `solid_queue`).
    ...
    - Otras áreas de discrepancia entre local y coolify/Coolify:
      ...
      2. **Bases de datos separadas (Pools de Conexión):** En coolify se usan bases de datos separadas para `primary`, `cache`, `queue` y `cable`. En local se comparte la base de datos `fitloop_development`. Si en coolify la concurrencia es alta y el pool de conexiones en `database.yml` es muy bajo (por defecto 5), la app fallará.
      3. **Action Cable (Solid Cable):** En coolify se requiere que el esquema de Solid Cable esté cargado (`bin/rails db:schema:load:cable`), de lo contrario Action Cable fallará al intentar reportar el progreso del anidado en tiempo real.
      4. **GeoIP / Cloudflare:** El paywall y checkout cambian de CRC (SINPE CRC) a USD (tarjeta) según el país (`CF-IPCountry` de Cloudflare). Si el proxy de Cloudflare está mal configurado en coolify o no se ha descargado el MMDB de GeoLite2 de respaldo, los usuarios fuera o dentro de Costa Rica verán divisas incorrectas.
      5. **Bypass de Webhooks en Cloudflare Zero Trust:** Si la app de coolify está protegida por Cloudflare Access (login obligatorio), ONVO no podrá entregar webhooks de pago exitoso a `/webhooks/onvo` a menos que se configure una regla de bypass para esa ruta exacta.
  </working_notes>
</task_session>
