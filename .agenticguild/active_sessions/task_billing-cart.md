...
<implementation_plan>
  ...
  <phase id="P6" name="Edge cases + regression tests">
    <step status="pending">Add request specs for: cupo plan download bypass; no nesting_run hides download option; overage pricing displayed; replace-confirm; redirect `/planes`; cart persistence across logout/login. (Started: added failing `spec/requests/cart_persistence_login_spec.rb` for guest_token session + merge-on-login)</step>
