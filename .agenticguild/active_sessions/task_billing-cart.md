...
<implementation_plan>
  ...
  <phase id="P8" name="Checkout flow refactor (method-first, dynamic receipt, single CTA)">
    ...
    <step id="P8.4" status="complete">Add i18n keys for new labels (“Ahorra ₡X”, “Procesar pago”, etc.) with `es_panic` parity; update CSS for the new card selector + receipt layout.</step>
    <step id="P8.5" status="complete">Run the focused spec set again and fix any regressions.</step>
  </phase>
</implementation_plan>
