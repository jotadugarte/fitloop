...
<implementation_plan>
  ...
  <phase id="P6" name="Edge cases + regression tests">
    <step status="pending">Add request specs for: cupo plan download bypass; no nesting_run hides download option; overage pricing displayed; replace-confirm; redirect `/planes`; cart persistence across logout/login. (Progress: implemented `session[:cart_guest_token]` persistence + merge-on-login hook; `spec/requests/cart_persistence_login_spec.rb` now passing; added paywall regression spec for no downloadable run hiding single-download CTAs; added checkout regression spec asserting explicit overage prices are rendered)</step>
